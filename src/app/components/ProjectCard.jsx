...
const ProjectCard = ({ imgUrl, title, description, link }) => {
  ...
  return (
    <div className='shadow-xl rounded-xl'>
        ...
        <div className='text-black rounded-b-xl bg-[#e3e3e3] px-4 py-6 group shadow-[inset_-12px_-8px_40px_#46464620]'>
            <h5 className='text-[1rem] font-semibold mb-2'>{title}</h5>
            <p className='font-bold text-[0.7rem] text-blue-500'>{description}</p>
        </div>
    </div>
  )
}
